package cn.itcast.review_11_多态复习.emp;
...
public class TestEmployees {
    public static void main(String[] args) {
        ...
        // 遍历数组，查看每个对象的信息，就是调用他们的getInfo方法
        //并统计实发工资总额，通知财务准备资金
        double sum = 0;
...
//          instanceof的作用是判断某个变量/对象的运行时类型是否<=instanceof后面写的类型。
            if (arr[i] instanceof SalaryEmployee) {//因为Manager是SalaryEmployee的子类，它的对象也会满足这个if条件
                SalaryEmployee se = (SalaryEmployee) arr[i]; // 只看arr[i]的编译时类型是Employee，不管arr[i]里面存储的是什么对象，这句代码都是向下转型
                                                            //向上，向下转型都是针对编译时类型
