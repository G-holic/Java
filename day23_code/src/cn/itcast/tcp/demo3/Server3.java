...
public class Server3 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) {
            ...
            // 启动线程
            new ClientHandThread(socket).start();
        }


        //不关闭，因为要接受n个客户端连接，一直工作
//        server.close();

    }
}

class ClientHandThread extends Thread {
    private Socket socket;

    public ClientHandThread(Socket socket) {
        this.socket = socket;
    }

    @Override
    public void run() { // 重写方法，编译时异常不允许，所有只能try..catch
        try (
                // (3)接收客户端发过来的单词或词语，然后我把它“反转”
