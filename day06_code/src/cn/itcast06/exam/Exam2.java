...
public class Exam2 {
    public static void main(String[] args) {
        int[] letter_counts = new int[26];//元素默认值是0
        /*
        ...
        while (true) {
            ...
            if (letter >= 'a' && letter <= 'z') {
                letter_counts[letter - 97]++;
            } else if (letter == '0') { // 或者 letter == 48
                break;
            }
        }
        for (int i = 0; i < letter_counts.length; i++) {
            if (letter_counts[i] != 0) { // 只显示次数部位0的
                System.out.println((char) (i + 'a') + ":" + letter_counts[i]);
            }
        }
