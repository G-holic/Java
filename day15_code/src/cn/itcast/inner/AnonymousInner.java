...
public class AnonymousInner {
    public static void main(String[] args) {
        new Object(); // 创建Object类的对象，没有产生新的类，使用已有的java.lan.Object类

        new Object() { // 创建Object的匿名子类对象，会产生新的类
            private int a;
            public void method(){
        ...
        new Father(){// 创建Father类的匿名对象，调用父类的无参构造

        };

        new Father("李四"){// 创建Father类的匿名对象，调用父类的有参构造

        };

        new Cloneable(){ // 创建Cloneable接口的匿名实现类对象，调用父类的无参构造 // 看字节码文件可以看到多了一个内部类
