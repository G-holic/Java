...
public class Exercise1 {
    public static void main(String[] args) {
        ...
        //（3）查看100以内的质数个数有几个
        System.out.println("100以内的质数的个数有: " + primeNumbers.size()); // 25
...
//                }
//                return false;
                return num % 10 == 7; // 删除返回true的
            }
        });

        //（8）再次使用Iterator遍历primeNumbers集合中剩下的质数。
        iterator = primeNumbers.iterator();// 保持迭代器都是在最新的元素更新后
        while (iterator.hasNext()) {
        ...
        for (int i = 1; i <= 10; i++) {
            int num = random.nextInt(100);// 产生[0,100]内的随机整数
            randNumbers.add(num);
        }
